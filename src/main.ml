...
let parse_and_print lexbuf =
  ...
  | Some { name = name; principals = p; knowledge = k; protocol = g; functions = f; equations = eq } ->
    let f = ("fst", (1, false))::("snd", (1, false))::("pair", (2, false)):: f in
    let env = List.map (fun (p, x) -> p, []) p in
    let errors = Typecheck.check g env [] f in
    let rules = translate name p k g f eq in
    ()
    (* print_errors errors; *)
